...
from models.hand_net import EncoderTransformer, EncoderTransformerCoarse, EncoderTransformerHRNet, EncoderTransformerInception
import numpy as np
import pickle
import os
...
class Trainer():
    def __init__(self, opt):
        ...
        if opt.net == 'reg_performer':
            ...
            print("[iccv2021] reg_transformer")
            self.net = EncoderTransformer(opt, self.mean_params).cuda()
        elif opt.net == 'reg_transformer_coarse':
            print("[iccv2021] reg_transformer_coarse")
            self.net = EncoderTransformerCoarse(opt, self.mean_params).cuda()
        elif opt.net == 'backbone_hrnet':
            self.net = EncoderTransformerHRNet(opt, self.mean_params).cuda()
        elif opt.net == 'backbone_incepv3':
            self.net = EncoderTransformerInception(opt, self.mean_params).cuda()
        ...
        for epoch in range(self.epoches):  # loop over the dataset multiple times
            ...
            for i, datas in enumerate(self.train_loader):
                ...
                for data in datas:
                    ...
                    if self.pl:
                        outputs,  _, pl_term, attn = self.net(inputs)
                    else:
                        outputs, _, attn = self.net(inputs)
