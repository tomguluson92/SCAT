...
class VIBELoss(nn.Module):
    ...
    def forward(
        ...
        batch_size,seqlen = pred_theta.shape[:2]
        ...
        if use_smooth:
            # <======== frame smooth Loss
            smooth_pose_loss = self.batch_smooth_pose_loss(pred_theta[:,:,3:51]) * self.e_pose_loss_weight *10
            smooth_shape_loss = self.batch_smooth_shape_loss(pred_theta[:,:,51:]) * self.e_shape_loss_weight *100
            loss_dict['smooth_pose_loss']=smooth_pose_loss
            loss_dict['smooth_shape_loss']=smooth_shape_loss
        ...
        use_smooth=False  #0121 change
        if use_smooth:
            # <======== frame smooth Loss
            smooth_3d_loss = self.batch_smooth_joints_loss(pred_j3d.reshape(batch_size,seqlen,21,3)) * 10000
            loss_dict['smooth_3d_loss']=smooth_3d_loss
            smooth_2d_loss = self.batch_smooth_joints_loss(pred_j2d.reshape(batch_size,seqlen,21,2)) * self.e_loss_weight/10
            loss_dict['smooth_2d_loss']=smooth_2d_loss
        ...
        real_shape, pred_shape = data_3d_theta[:, 51:], pred_theta[:, 51:]
        real_pose, pred_pose = data_3d_theta[:, 3:51], pred_theta[:, 3:51]
        real_shape=torch.zeros(data_3d_theta.shape[0], 10).to(self.device).float()
        loss_dict['loss_kp_2d']=loss_kp_2d
        ...
        if pred_theta.shape[0] > 0:
            ...
            loss_pose, loss_shape = self.mano_losses(pred_pose, pred_shape, real_pose, real_shape)
    ...
    def keypoint_loss(self, pred_keypoints_2d, gt_keypoints_2d, openpose_weight, gt_weight):
        ...
        #conf[:, 25:] *= gt_weight
        #loss = self.criterion_keypoints(pred_keypoints_2d, gt_keypoints_2d).mean()  #VIBE
        loss = nn.L1Loss(reduction='none')(pred_keypoints_2d,gt_keypoints_2d).mean()   #frankmocap
        return loss
    ...
    def smpl_losses(self, pred_rotmat, pred_betas, gt_pose, gt_betas):
        pred_rotmat_valid = batch_rodrigues(pred_rotmat.reshape(-1,3)).reshape(-1, 16, 3, 3)
        gt_rotmat_valid = batch_rodrigues(gt_pose.reshape(-1,3)).reshape(-1, 16, 3, 3)
    ...
    def mano_losses(self, pred_pose, pred_betas, gt_pose, gt_betas):
        loss_regr_pose=self.criterion_regr(pred_pose, gt_pose)
        loss_regr_betas = self.criterion_regr(pred_betas, gt_betas)
        return loss_regr_pose, loss_regr_betas

    def batch_smooth_pose_loss(self, pred_theta):
        #pose = pred_theta[:,:,3:75]
        pose = pred_theta
        # abs ========>
        rot_diff = pose[:,1:,:3]-pose[:,:-1,:3]
        pose_diff = pose[:,1:,3:] - pose[:,:-1,3:]
        loss = torch.mean(pose_diff).abs()+torch.mean(rot_diff).abs()*10
        # MSE loss ======>
        #loss=self.criterion_regr(pose[:,1:,:], pose[:,:-1,:])
        return loss


    def batch_smooth_shape_loss(self, pred_theta):
        #shape = pred_theta[:, :, 75:]
        shape = pred_theta
        # abs ========>
        shape_diff = shape[:, 1:, :] - shape[:, :-1, :]
        loss = torch.mean(shape_diff).abs()
        # MSE loss ======>
        #loss=self.criterion_regr(shape[:,1:,:], shape[:,:-1,:])
        return loss
    
    def batch_smooth_joints_loss(self, joints):
        '''
            Inputs:
                3d joints: batch_size x seqlen x 21 x 3
                2d joints: batch_size x selen x 21 x 2
        '''
        reduce = lambda x: x.reshape((x.shape[0] * x.shape[1],) + x.shape[2:])
        #rot=joints[:,:,0]
        #joints=joints[:,:,1:]
        # abs ========>
        #joints_diff = reduce(joints[:, 1:, :]) - reduce(joints[:, :-1, :])
        #loss = torch.mean(joints_diff).abs()
        # MSE loss ======>
        #rot_loss=self.criterion_regr(reduce(rot[:,1:,:]), reduce(rot[:,:-1,:]))*5
        loss=self.criterion_regr(reduce(joints[:,1:,:]), reduce(joints[:,:-1,:]))
        #loss=loss+rot_loss
